// ============================================================
// Automa CRM public API client.
//
// A thin wrapper over the `/api/v1` REST surface. It attaches the
// bearer key, unwraps the `{ data }` / `{ error }` envelope, and
// turns API failures into a typed AutomaCrmApiError the tools can render
// cleanly. Nothing here knows about MCP — it's just the CRM API.
// ============================================================

import type { Config } from './config.js';

/** A structured error from the Automa CRM API envelope (`{ error: { code, message } }`). */
export class AutomaCrmApiError extends Error {
  readonly status: number;
  readonly code: string;

  constructor(status: number, code: string, message: string) {
    super(message);
    this.name = 'AutomaCrmApiError';
    this.status = status;
    this.code = code;
  }
}
...
export class AutomaCrmClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new AutomaCrmApiError(
        0,
        'network_error',
        `Could not reach Automa CRM at ${this.baseUrl}: ${(err as Error).message}`,
      );
    }
    ...
    if (text) {
      try {
        ...
        if (!res.ok) {
          throw new AutomaCrmApiError(res.status, 'internal', text.slice(0, 500));
        }
      }
    }
    ...
    if (!res.ok) {
      ...
      throw new AutomaCrmApiError(res.status, code, message);
    }
