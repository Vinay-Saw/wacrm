...
// layer dumb). On an AutomaCrmApiError we return an `isError` result with
...
import { AutomaCrmApiError } from '../client.js';
...
/**
 * Wrap a tool handler so any AutomaCrmApiError becomes a clean, model-
...
export function handle<A>(
  ...
  return async (args: A) => {
    try {
      ...
      if (err instanceof AutomaCrmApiError) {
        return errorResult(`Automa CRM API error [${err.code}]: ${err.message}`);
      }
