# Automa CRM MCP server

A [Model Context Protocol](https://modelcontextprotocol.io) server for
**Automa CRM** — the self-hostable WhatsApp CRM. It lets MCP clients
(Claude Desktop, Claude Code, Cursor, and others) drive your CRM in
natural language:
...
It's a thin wrapper over Automa CRM's public [`/api/v1`](../docs/public-api.md)
REST API. All auth, scoping, and rate limiting are enforced by your
Automa CRM instance — this server just exposes the API as MCP tools.

## Prerequisites

1. A running Automa CRM instance (your own self-hosted deploy).
...
| ------------------------- | -------- | -------------------------------------------------------------- |
| `AUTOMA_CRM_BASE_URL`     | yes      | Your instance URL, e.g. `https://crm.example.com`              |
| `AUTOMA_CRM_API_KEY`      | yes      | An API key from the dashboard                                  |
| `AUTOMA_CRM_ENABLE_WRITES`| no       | `true` to expose contact writes + message sending             |
| `AUTOMA_CRM_ENABLE_BROADCASTS` | no  | `true` to expose mass broadcasts (needs `AUTOMA_CRM_ENABLE_WRITES`) |
...
```jsonc
{
  "mcpServers": {
    "automacrm": {
      ...
      "env": {
        "AUTOMA_CRM_BASE_URL": "https://crm.example.com",
        "AUTOMA_CRM_API_KEY": "automacrm_live_xxxxxxxxxxxxxxxxxxxxxxxx"
      }
    }
  }
}
...
"env": {
  "AUTOMA_CRM_BASE_URL": "https://crm.example.com",
  "AUTOMA_CRM_API_KEY": "automacrm_live_xxxxxxxxxxxxxxxxxxxxxxxx",
  "AUTOMA_CRM_ENABLE_WRITES": "true",
  "AUTOMA_CRM_ENABLE_BROADCASTS": "true"
}
```
...
1. **Read-only by default.** Write and broadcast tools are not even
   ...
   `AUTOMA_CRM_ENABLE_WRITES` / `AUTOMA_CRM_ENABLE_BROADCASTS`.
2. **API-key scopes.** Whatever the guards allow, your Automa CRM instance
