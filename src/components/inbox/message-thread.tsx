...
export function MessageThread({
  ...
  const { user } = useAuth();
  const { data: queryMessages, isLoading: messagesLoading } = useMessages(
    conversation?.id,
    { resyncToken }
  );
  ...
  // Sync messages fetched by React Query with parent
  useEffect(() => {
    if (queryMessages && conversationId) {
      onMessagesLoadedRef.current(queryMessages);
    }
  ...
  return (
    <div className={cn('flex min-w-0 flex-1 flex-col', DOODLE_BG_CLASSES)}>
      <ThreadHeader
        ...
        onBack={onBack}
        onRefreshClick={onRefresh ? handleRefreshClick : undefined}
