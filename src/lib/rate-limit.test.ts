...
describe('checkRateLimitRedis & checkRateLimitAsync', () => {
  ...
  it('checkRateLimitAsync works seamlessly with in-memory fallback', async () => {
    const { checkRateLimitAsync } = await import('./rate-limit');
    const result = await checkRateLimitAsync('async:user:1', {
      limit: 2,
      windowMs: 60_000,
    });
    expect(result.success).toBe(true);
    ...
    const result2 = await checkRateLimitAsync('async:user:1', {
      limit: 2,
      windowMs: 60_000,
    });
    ...
    const result3 = await checkRateLimitAsync('async:user:1', {
      limit: 2,
      windowMs: 60_000,
    });
